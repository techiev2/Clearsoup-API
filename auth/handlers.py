...
class Authenticate(BaseHandler):
    ...
    def post(self, *args, **kwargs):
        ...
        if self.get_argument('google_oauth', None):
            _oauth, _provider = self.get_argument(
                'google_oauth', None), 'google'

        if self.get_argument('github_oauth', None):
            _oauth, _provider = self.get_argument(
                'github_oauth', None), 'github'
        ...
        if _oauth:
            ...
            user = SessionManager.validateLogin(
        ...
        if user:
            ...
            # This is because we will be signing the token with
            # RequestHandler.create_signed_value to prevent tampering
